...
const _GetBoardStates = () => {
	...
	for (let x = 0; x < 3; x++) {
		...
		boardStates.push(row); // ? [[각 좌표의 결과 값], []...]
	}
	return boardStates;
};

const _GetSquareElementNodes = () => {
	const nodes = [];
	for (let x = 0; x < 3; x++) {
		for (let y = 0; y < 3; y++) {
...
const _HighlightSquares = (blinks) => {
	if (blinks === undefined) {
		blinks = 10;
	}
...
const _EvaluateBoard = (boardStates) => {
	const winningStates = [
		// Horizontals
		[
			[0, 0],
	...
	// ? 가능한 승리의 경우의 수 중 curPlayer 값이 나중에 오는 occupant의 값과
	// ? 일치 시 동일 플레이어가 승리 찬스를 계속 유지하는 것으로 간주
	...
	// ? 더 이동할 곳이 있는지 확인 후 확인할 곳이 없으면 draw 판정을 return
	let hasMoves = false;
	for (let x = 0; x < 3; x++) {
		for (let y = 0; y < 3; y++) {
...
// ? Minimax_Maximize와 Minimax_Minimize를 번갈아 돌려가며
// ? 최종적으로 이길 수 있는 수를 계속해서 찾음
const _Minimax = (boardStates, player) => {
...
// ? newBoardStates는 상위 Parent로 계속해서 저장된 정보를 context처럼 전달
// ? player가 놓을 수 있는 모든 위치 값에 대한 결과를 분석(예상 승자의 결과가 나올 때까지)
// ? 존재하는 blank에 ai의 가상 값을 할당한 뒤,
// ? minimax 함수에 대입하여 승리가 가능한지 비교
// ? 승리가 가능할 시 그곳에 수를 둠
const _Minimax_Maximize = (boardStates) => {
...
// ? 존재하는 blank에 player의 가상 값을 할당한 뒤,
// ? minimax 함수에 대입하여 패배하게 되는지 비교
const _Minimax_Minimize = (boardStates) => {
	...
	for (let x = 0; x < 3; x++) {
		for (let y = 0; y < 3; y++) {
			if (boardStates[x][y] == BOARD_STATE.blank) {
				...
				// ? 최종적으로 지는 경우가 나올 경우 그곳을 막기 위해 수를 둠
				if (newMoveScore < moveScore) {
					move = [x, y];
					moveScore = newMoveScore;
				}
...
const _Minimax2 = (boardStates, aiTurn) => {
	// First check if the game has already been won.
	const winner = _EvaluateBoard(boardStates);
	if (winner == BOARD_STATE.ai) {
		return [1, null];
